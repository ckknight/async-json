(function (exports, undefined) {
    ...
    function stringifyAux(data, callback, key) {
        switch (typeof data) {
        case "undefined":
        ...
        case "object":
            if (data === null) {
                ...
                if (typeof then === 'function') {
                    ...
                    stringify(data.toJSON(key), callback, key);
                } else if (isPrimitiveConstructor(data.constructor)) {
                    // horrible, someone used the new String(), new Number(), or new Boolean() syntax.
                    stringify(data.valueOf(), callback, key);
                } else if (isArray(data)) {
                    internalStringifyArray(data, callback);
                } else {
                    internalStringifyObject(data, callback);
                }
            }
            break;
        case "function":
    ...
    function stringify(data, callback, key) {
        try {
            stringifyAux(data, callback, key);
        } catch (err) {
            callback(err);
        }
    }
